...
// Windows can only execute a .cmd/.bat file via cmd.exe, and @openvidstudio/capture's
// spawnCapture routes that one case through shell:true, so cmd.exe's own metacharacters
// are in scope for any argument reaching that spawn call.
...
/**
 * Child-process spawning now lives in @openvidstudio/capture, which owns every code path
 * that shells out. Re-exported here because roughly twenty call sites in this package
 * import it from "../util", and the boundary that moved is a packaging boundary, not a
 * call-site one.
 */
export { spawnCapture } from "@openvidstudio/capture";
export type { SpawnResult } from "@openvidstudio/capture";
